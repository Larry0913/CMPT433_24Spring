...
static void fillPlaybackBuffer(short *buff, int size)
{
	...
	// Step 1: Wipe the playbackBuffer to all 0's to clear any previous PCM data
    memset(buff, 0, sizeof(short) * size);

	// Step 2: Synchronize access to the soundBites array
    pthread_mutex_lock(&audioMutex);
	{
		// Step 3: Loop through each slot in soundBites[]
		for (int i = 0; i < MAX_SOUND_BITES; i++) {
			if (soundBites[i].pSound != NULL) { // If the sound bite slot is used
			
				for (int j = 0; j < size && (j + soundBites[i].location) < soundBites[i].pSound->numSamples; j++) {
					// Calculate the new value with potential overflow/underflow handling
					int newValue = buff[j] + soundBites[i].pSound->pData[j + soundBites[i].location];
					if (newValue > SHRT_MAX) {
						newValue = SHRT_MAX;
					} else if (newValue < SHRT_MIN) {
						newValue = SHRT_MIN;
					}
					buff[j] = (short)newValue;
				}

				soundBites[i].location += size;
				
				if (soundBites[i].location >= soundBites[i].pSound->numSamples) {
					// We've played back the entire sample, so mark this slot as free
					soundBites[i].pSound = NULL;
