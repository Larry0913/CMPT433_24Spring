...
void getJoystick(void) {
    if (!(__R31 & JOYSTICK_RIGHT_MASK)) {
        pSharedMemStruct->pruJoystickRight = 0;    //if pressed
    }
    else {
        pSharedMemStruct->pruJoystickRight = 1;    //not pressed
    }
    if (!(__R31 & JOYSTICK_DOWN_MASK)) {
        pSharedMemStruct->pruJoystickDown = 0;     //if pressed
    }
    else {
        pSharedMemStruct->pruJoystickDown = 1;     //not pressed
    }
}
...
void main(void)
{
    ...
    pSharedMemStruct->pruJoystickDown = 1; //not press
    pSharedMemStruct->pruJoystickRight = 1; //not press
