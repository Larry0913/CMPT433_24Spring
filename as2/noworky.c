...
{
	// The bug was in the tradeArrays function, where it was decrementing an unsigned int from 0 in a for loop, 
	// causing it to underflow and wrap around to the maximum value, leading to out-of-bounds memory access. 
	// It should have been either using a signed integer for the loop variable or iterating 
	// from 0 to size - 1 to safely swap the contents of the two arrays without causing an underflow.
	//unsigned int i;
	int i;
	for (i = size-1; i >= 0; i--) {
